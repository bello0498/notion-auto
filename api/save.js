...
module.exports = async (req, res) => {
  ...
  if (!NOTION_TOKEN || !NOTION_DATABASE_ID || !NOTION_PAGE_ID) {
    return res.status(500).json({ error: "Missing NOTION_TOKEN or NOTION_DATABASE_ID or NOTION_PAGE_ID environment variables" });
  }

  const body = await readJSON(req);
  const { mode, title, content, url, date, tags, status } = body || {};
  ...
  try {
    ...
    if (mode === "page") {
      // ----- Page 저장 (하위 페이지 생성 + 블록 추가) -----
      if (!title) return res.status(400).json({ error: "Missing 'title' in request body (Page mode)" });
      ...
      return res.status(200).json({ ok: true, mode: "page", pageId: childPage.id });
    }
